...
########################################################################
my $plist = Mac::PropertyList::Foundation::parse_plist( $array );

isa_ok( $plist, 'Mac::PropertyList::Foundation::array' );
is(     $plist->type, 'array', 'Item is an array type' );
isa_ok( $plist->value, 'Mac::PropertyList::Foundation::array' );

{
my @elements = @{ $plist->value };
isa_ok( $elements[0], 'Mac::PropertyList::Foundation::string' );
isa_ok( $elements[1], 'Mac::PropertyList::Foundation::string' );
...
isa_ok( $plist->value, 'Mac::PropertyList::Foundation::dict' );

{
my $hash = $plist->value;
ok( exists $hash->{Mimi},           'Mimi key exists for dict'         );
isa_ok( $hash->{Mimi}, 'Mac::PropertyList::Foundation::string' );
...
foreach my $string ( ( $string0_9, $string1_0 ) )
	{
	my $plist = Mac::PropertyList::Foundation::parse_plist( $string );

	isa_ok( $plist, 'Mac::PropertyList::Foundation::string' );
	is( $plist->type, 'string',      'type key has right value for string' );
	is( $plist->value, 'This is it', 'value is right for string'           );
	}
	
$plist = Mac::PropertyList::Foundation::parse_plist( $nested_dict );
