...
foreach my $start ( ( $array, $dict ) )
	{
	my $plist  = Mac::PropertyList::Foundation::parse_plist( $start );
	my $string = Mac::PropertyList::Foundation::plist_as_string( $plist );
	is( $string, $start, 'Original and rewritten string match' );
	}

my $plist  = Mac::PropertyList::Foundation::parse_plist( $nested_dict );
my $string = Mac::PropertyList::Foundation::plist_as_string( $plist );
